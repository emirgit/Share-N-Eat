...
const FindYourMealModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateInputs()) {
      ...
      if (nutritionValues.carbs.min) queryParams.append('minCarbs', nutritionValues.carbs.min);
      if (nutritionValues.carbs.max) queryParams.append('maxCarbs', nutritionValues.carbs.max);
      
      if (nutritionValues.fat.min) queryParams.append('minFat', nutritionValues.fat.min);
      if (nutritionValues.fat.max) queryParams.append('maxFat', nutritionValues.fat.max);
      
      if (nutritionValues.protein.min) queryParams.append('minProtein', nutritionValues.protein.min);
      if (nutritionValues.protein.max) queryParams.append('maxProtein', nutritionValues.protein.max);
      
      if (nutritionValues.calories.min) queryParams.append('minCalories', nutritionValues.calories.min);
      if (nutritionValues.calories.max) queryParams.append('maxCalories', nutritionValues.calories.max);
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[9999] bg-black bg-opacity-50 backdrop-filter backdrop-blur-sm flex items-center justify-center pointer-events-auto">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl overflow-y-auto max-h-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex space-x-4">
              <input
                ...
                onChange={(e) => setNutritionValues(prev => ({
                  ...prev,
          ...
          {errors.totalNutrients && <p className="text-red-500 font-medium">{errors.totalNutrients}</p>}
