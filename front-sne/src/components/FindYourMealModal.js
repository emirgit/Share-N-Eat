...
const FindYourMealModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  // Render the modal at the top level of the DOM
  return ReactDOM.createPortal(
    <div
      className="
        ...
        backdrop-filter backdrop-blur-sm /* Blurs the background */
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl overflow-y-auto max-h-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {errors.carbsMin && (
              <p className="text-red-500 text-sm">{errors.carbsMin}</p>
            )}
            {errors.carbsMax && (
              <p className="text-red-500 text-sm">{errors.carbsMax}</p>
            )}
            {errors.carbsRange && (
              <p className="text-red-500 text-sm">{errors.carbsRange}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.proteinMin && (
              <p className="text-red-500 text-sm">{errors.proteinMin}</p>
            )}
            {errors.proteinMax && (
              <p className="text-red-500 text-sm">{errors.proteinMax}</p>
            )}
            {errors.proteinRange && (
              <p className="text-red-500 text-sm">{errors.proteinRange}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.caloriesMin && (
              <p className="text-red-500 text-sm">{errors.caloriesMin}</p>
            )}
            {errors.caloriesMax && (
              <p className="text-red-500 text-sm">{errors.caloriesMax}</p>
            )}
            {errors.caloriesRange && (
              <p className="text-red-500 text-sm">{errors.caloriesRange}</p>
            )}
          </div>

          {/* Total Nutrients Error */}
          {errors.totalNutrients && (
            <p className="text-red-500 font-medium">{errors.totalNutrients}</p>
          )}

          {/* General Error (no fields specified) */}
